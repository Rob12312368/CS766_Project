...
Adapter: LoRA(attention, r=64) + linear probe
...
model.decode_head.classifier = torch.nn.Conv2d(768, 20, kernel_size=(1, 1), stride=(1, 1))
...
def denormalize(tensor):
    ...
    return tensor
def plot_segmentation(images, targets, preds, save_dir, counter = 0):
    ...
    plt.savefig(save_dir + '/segmentation_' + str(counter) + '.png')
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for images, masks in val_loader:
            ...
            if counter == 0 or counter == 1 or counter == 2 or counter == 3 or counter == 4 or counter == 5:
                plot_segmentation(images, masks, predicted, save_dir, counter)
            counter += 1
