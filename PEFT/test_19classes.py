...
Adapter: None(fine-tuned on Cityscapes, base model, train linear probe)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for batch in val_loader:
            for images, masks in batch:
                ...
                plot_segmentation(images, masks, predicted, save_dir)

                total += targets.nelement()  #
                correct += (predicted == targets).sum().item()
                # save the distribution of masks and predicted values
                plt.figure()
                plt.hist(targets.cpu().numpy().flatten(), bins=20, alpha=0.5, label='targets')
                plt.savefig(save_dir + '/hist0.png')
                plt.figure()
                plt.hist(predicted.cpu().numpy().flatten(), bins=20, alpha=0.5, label='predicted')
                plt.savefig(save_dir + '/hist1.png')


                iou_per_class = []
                for cls in range(19):  # Consider classes 0 to 18
                    predicted_cls = predicted == cls
                    target_cls = targets == cls
                    intersection = (predicted_cls & target_cls).sum().item()

                    # Exclude when predicted is class 'cls' and target is class 19 from the union calculation
                    exclude = (predicted_cls & (targets == 19)).sum().item()
                    union = (predicted_cls | target_cls).sum().item() - exclude

                    if union == 0:
                        iou_per_class.append(float('nan'))  # Avoid division by zero
                    else:
                        iou_per_class.append(intersection / union)
                    print(f"Class {cls}, predicted: {predicted_cls.sum().item()}, target: {target_cls.sum().item()}, intersection: {intersection}, union: {union}, IoU: {100 * iou_per_class[-1]:.2f}%")


                mean_iou = np.nanmean(iou_per_class)
                total_mIou.append(mean_iou)
                print(f"Batch {counter}, Mean IoU: {100 * mean_iou:.2f}%")
                exit()

            if counter % 10 == 0:
                end_time = time.time()
                duration = end_time - start_time
                print(f"Batch {counter}, Mean IoU: {100 * mean_iou:.2f}%, Test time: {duration:.2f} s")
                logger.info( f"Batch {counter}, Mean IoU: {100 * mean_iou:.2f}%, Test time: {duration:.2f} s")
            counter += 1
    ...
    print(f"Epoch {epoch + 1}, accuracy: {accuracy*100:.2f}%, mIOU: {avg_mIou*100:.2f}%, Epoch Duration: {epoch_duration} s")
    logger.info(f"Epoch {epoch + 1},  accuracy: {accuracy*100:.2f}%, mIOU: {avg_mIou*100:.2f}%, Epoch Duration: {epoch_duration} s")
